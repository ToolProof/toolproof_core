...
import { Job, Workflow, WorkflowStepUnion, DataExchange } from 'updohilo/dist/types/typesWF';
...
interface StepPropertiesPanelProps {
    ...
    availableJobs: Job[];
    workflow: Workflow;
    onUpdateStep: (step: WorkflowStepUnion) => void;
    onClose: () => void;
}
...
export default function StepPropertiesPanel({
    ...
    const [localStep, setLocalStep] = useState<WorkflowStepUnion>(step);
    ...
    const renderSimpleStepProperties = () => {
        if (localStep.type !== 'simple') return null;

        const job = availableJobs.find(j => j.id === localStep.step.jobId);
        if (!job) return <div className="text-red-600">Job not found</div>;

        const updateDataExchange = (index: number, field: keyof DataExchange, value: string) => {
            const newExchanges = [...localStep.step.dataExchanges];
        ...
        const addDataExchange = () => {
            const newExchange: DataExchange = {
                sourceJobId: '',
                sourceOutput: '',
    ...
    const renderParallelStepProperties = () => {
        if (localStep.type !== 'parallel') return null;
        
        return (
            <div className="space-y-4">
                <h3 className="text-lg font-medium text-gray-900">Parallel Execution</h3>
                <p className="text-sm text-gray-600">
                    This step executes multiple branches in parallel.
                </p>
                <div className="text-sm text-gray-700">
                    Branches: {localStep.branches.length}
                </div>
                {/* TODO: Add branch management UI */}
            </div>
        );
    };

    const renderConditionalStepProperties = () => {
        if (localStep.type !== 'conditional') return null;
        
        return (
            <div className="space-y-4">
                <h3 className="text-lg font-medium text-gray-900">Conditional Step</h3>
                <p className="text-sm text-gray-600">
                    This step executes different branches based on conditions.
                </p>
                {/* TODO: Add condition management UI */}
            </div>
        );
    };
    ...
    return (
        <div className="h-full flex flex-col bg-white">
            ...
            <div className="flex-1 overflow-y-auto p-6">
                {step.type === 'simple' && renderSimpleStepProperties()}
                {step.type === 'parallel' && renderParallelStepProperties()}
                {step.type === 'conditional' && renderConditionalStepProperties()}
                {(step.type === 'while' || step.type === 'for') && (
                    <div className="space-y-4">
                        <h3 className="text-lg font-medium text-gray-900">
                            {step.type === 'while' ? 'While Loop' : 'For Loop'}
                        </h3>
                        <p className="text-sm text-gray-600">
